...
void initKeyMapping()
{
    ...
    keyMap.push_back(make_pair("condition",CONDITION));
    keyMap.push_back(make_pair("configuration",CONFIGURATION));
    keyMap.push_back(make_pair("action",ACTION));
    keyMap.push_back(make_pair("case",CASE));
    keyMap.push_back(make_pair("if",IF));
    keyMap.push_back(make_pair("transaction",TRANSACTION));
    keyMap.push_back(make_pair("port",PORT));
    keyMap.push_back(make_pair("direct",DIRECT));
    keyMap.push_back(make_pair("width",WIDTH));
    keyMap.push_back(make_pair("storange",STORANGE));
    keyMap.push_back(make_pair("type",TYPE));
    keyMap.push_back(make_pair("size",SIZE));
    keyMap.push_back(make_pair("property",PROPERTY));
...
void preProcess(char * word,int line)
{
    ...
    int flag = 0;
    p_include = strstr(word,include_temp);//include这段可以删的
    if(p_include!=NULL)
    {
        flag = 1;
        int i;
